...
class API extends Base {
   ...
   * @memberof API
   */
  async addDomain(appId, { requestdomain, wsrequestdomain, uploaddomain, downloaddomain }) {
    const token = await this.oauth.ensureAccessToken(appId);
    const url = this.wxaPrefix + 'modify_domain?access_token=' + token.accessToken;
    const data = {
      action: 'add',
   ...
   * @memberof API
   */
  async delDomain(appId, { requestdomain, wsrequestdomain, uploaddomain, downloaddomain }) {
    const token = await this.oauth.ensureAccessToken(appId);
    const url = this.wxaPrefix + 'modify_domain?access_token=' + token.accessToken;
    const data = {
      action: 'delete',
   ...
   * @memberof API
   */
  async addWebviewDomain(appId, webviewdomain) {
    const token = await this.oauth.ensureAccessToken(appId);
    const url = this.wxaPrefix + 'setwebviewdomain?access_token=' + token.accessToken;
    const data = {
      action: 'add',
   ...
   * @memberof API
   */
  async delWebviewDomain(appId, webviewdomain) {
    const token = await this.oauth.ensureAccessToken(appId);
    const url = this.wxaPrefix + 'setwebviewdomain?access_token=' + token.accessToken;
    const data = {
      action: 'delete',
