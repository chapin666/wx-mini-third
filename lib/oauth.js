...
class OAuth extends Base {
  ...
  async code2Session(appId, code) {
    const componentTokenObj = await this.ensureComponentToken();
    const componentToken = componentTokenObj.componentAccessToken;
    const url = snsPrefix + `component/jscode2session?appid=${appId}&js_code=${code}&grant_type=authorization_code&component_appid=${this.appid}&component_access_token=${componentToken}`;
